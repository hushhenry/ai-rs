use zeroai::auth::{
    ...
    ProviderAuthInfo, config::ConfigManager,
};
use zeroai::oauth::{
        ...
        if event::poll(std::time::Duration::from_millis(50))? {
            if let Event::Key(key) = event::read()? {
                ...
                if let Screen::AuthInput(state) = &*screen {
                    if state.is_oauth && config.has_credential(&state.provider_id).unwrap_or(false) {
    ...
    match method {
        ...
        AuthMethod::OAuth { hint } => {
            ...
            *screen = Screen::AuthInput(AuthInputState {
                ...
                input: String::new(),
                hint: hint.unwrap_or_else(|| "Connecting...".into()),
                is_oauth: true,
                oauth_url: None,
            });
        }
...
async fn enter_model_selection(config: &ConfigManager, provider_id: &str, screen: &mut Screen) -> anyhow::Result<()> {
    ...
    let adapter = zeroai::adapter::AdapterKind::from_lower_str(provider_id);

    let client = zeroai::Client::default();
    let model_names: Vec<String> = if let Some(ak) = adapter {
        client.all_model_names(ak).await.unwrap_or_default()
    } else {
        Vec::new()
    };

    let model_items: Vec<(String, bool)> = model_names
        .into_iter()
        .map(|m| {
    ...
    match screen {
        Screen::ProviderGroups => {
            let items: Vec<ListItem> = groups
                .iter()
                .map(|(label, providers)| {
                    let has_any_cred = providers
                        .iter()
                        .any(|p| config.has_credential(&p.provider_id).unwrap_or(false));
